...
class MapActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        OpenpilotStateRepository.state.observe(this) { state ->
            ...
            if (state.ip != "-" && state.ip.isNotEmpty()) {
                ...
                // v4.13: "연결 대기에서 변화가 없어 앱이 멈춘 것처럼 보인다"(재억 6번) -
            ...
            override fun onSuccess() {
                runOnUiThread {
                    ...
                    try {
                        ...
                        // v4.8: 재억 요청 - 차선/신호등 데이터를 따로 주는 observable/메서드가
    ...
    private fun dismissKeyboardAndSearchPanel() {
        ...
        // 포커스가 안 풀리고, 그래서 검색창 커서(깜빡임)가 계속 남아있는 문제(재억 8번).
    ...
    private fun startSafeDriveMode() {
        ...
        navigationFragment?.let { frag ->
            ...
            */
            ...
            // v4.8: 재억 요청으로 티맵 쪽도 차선/신호등 데이터 소스 계속 조사. 공식 문서에서
            ...
            TmapUISDK.observableEDCData.observe(this@MapActivity, Observer { data ->
                data?.let {
                    ...
                    // 확인해서(재억 2·3·4번) LaneSignalRepository에 연결. 카카오가 이미
    ...
    // v4.13: 순정 Tmap 안내 시 콤마에 회전방향(좌/우/유턴)을 못 보내는 문제(재억 9번) -
    ...
    // v4.13: Tmap 자체 안내 시엔 지금까지 차선정보가 아예 안 나왔던 문제(재억 2·3·4번) -
    ...
    // 못 했음 - 잘못 추측해서 틀린 차선을 추천 표시하면 위험하므로(재억 지적 사례와 동일한
