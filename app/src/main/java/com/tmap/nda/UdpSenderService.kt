            ...
            serviceScope.launch {
                try {
                    ...
                    if (KakaoRouteDataRepository.isFresh(60_000L)) {
                        ...
                        // "카카오안내". #문제시 원복
                        // v: 재억 요청(2026-09-19) - 계기판(openpilot HUD) 카드 첫 줄이 다음
                        // 회전 지점명(tbtMainText, 예: "탄현로")으로 떠서 목적지 이름이 안
                        // 보인다는 제보. 목적지명을 최우선으로 보내고, 목적지명이 없을 때만
                        // 예전처럼 다음 회전 지점명 → 도로명 순으로 폴백. #문제시 원복
                        val kakaoPrefix = if (kr.destinationName.isNotBlank() && kr.destinationName != "목적지") {
                            kr.destinationName
                        } else if (kr.tbtMainText.isNotBlank()) {
                            kr.tbtMainText
