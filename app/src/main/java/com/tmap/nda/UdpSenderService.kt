...
class UdpSenderService : Service() {
    ...
    // 제한속도인 것처럼 오인식돼 "10% 안 넘었는데 경고음" 오탐이 발생했음(재억 지적).
        ...
        if (bundle != null && isRunning.get()) {
            ...
            serviceScope.launch {
                try {
                    ...
                    // 최신값을 읽을 수 있음(재억 요청: "카카오 화면에서도 티맵 정보 그대로").
                    // #문제시 원복
                    try {
                        val sdiType = json.optInt("nSdiType", 0)
                        val sdiSpeedLimit = json.optInt("nSdiSpeedLimit", 0)
                    ...
                    if (KakaoRouteDataRepository.isFresh()) {
                        ...
                        val kakaoPrefix = when {
                            ...
                            // "지산동(현재위치) 말고 대구광역시청(목적지)이 떠야지" (재억 지적 6번). #문제시 원복
            ...
            try {
                ...
                while (isActive && isRunning.get()) {
                    ...
                    try {
                        ...
                        // 안 됐음(재억 지적: 로그에 active= 기록이 거의 없다). 5초마다
                ...
                while (isActive && isRunning.get()) {
                    ...
                    try {
                        ...
                        if (json.has("location")) {
                            ...
                            // 재억 진단용: openpilot road_speed_limiter.py에서 GPS 패킷에 얹어보내는
